...
# ---------- LASSO ----------
#' Function for variables selection using the LASSO method
#'
...
#' @param family the distribution, always use "gaussian" here
#' @export
#' @import glmnet caret
#' @examples
#' select_features_lasso(feat_matrix=tenerife$comp_list, sel_factor=tenerife$species)
select_features_lasso <- function(feat_matrix, sel_factor, family="gaussian") {
...
# ---------- Elastic Net ----------
#' Function for variables selection using the Elastic Net method
#'
#' @param feat_matrix the matrix containing the molecular information, with samples in rows and molecular features in columns
#' @param sel_factor the factor which is used to select the variables
#' @param sel_colors color vector for the factor
#' @param confidence confidence level, defaults to 0.1
#' @param tune_length how often repeated cross validation is performed, defaults to 100
#' @param plot_roc_filename plot metrics to external pdf, defaults to NULL
#' @export
#' @import glmnet caret
#' @examples
#' select_features_elastic_net(feat_matrix=tenerife$comp_list, sel_factor=tenerife$species, sel_colors=sapply(tenerife$species, function(x) { x <- RColorBrewer::brewer.pal(n=nlevels(tenerife$species), name="Set1")[which(x==levels(tenerife$species))] } ))
select_features_elastic_net <- function(feat_matrix, sel_factor, sel_colors, confidence=0.1, tune_length=100, plot_roc_filename=NULL) {
	# Handle plotting
...
#' @import glmnet caret
#' @examples
#' select_features_pls(feat_matrix=tenerife$comp_list, sel_factor=tenerife$species, sel_colors=sapply(tenerife$species, function(x) { x <- RColorBrewer::brewer.pal(n=nlevels(tenerife$species), name="Set1")[which(x==levels(tenerife$species))] } ), components=5)
